...
export function SidebarDashboard({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { href: "/dashboard", label: "Agendamento", icon: <CalendarCheck2 className='w-6 h-6' /> },
    { href: "/dashboard/services", label: "Serviços", icon: <Folder className='w-6 h-6' /> },
    { href: "/dashboard/profile", label: "Meu perfil", icon: <Settings className='w-6 h-6' /> },
    { href: "/dashboard/plans", label: "Planos", icon: <Banknote className='w-6 h-6' /> }
  ]
          ...
          <Sheet>
            ...
            <SheetContent side="right" className="sm:max-w-xs text-black">
              ...
              <nav className='grid gap-2 text-base pt-5'>
                {navItems.map(item => (
                  <SidebarLink key={item.href} {...item} pathname={pathname} isCollapsed={false} />
                ))}
              </nav>
            </SheetContent>
          </Sheet>
